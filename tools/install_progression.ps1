<#
.SYNOPSIS
  Копирует конфиг прогрессии из src/cse/ в runtime/.

.DESCRIPTION
  Идемпотентный скрипт: повторный запуск безопасно перезаписывает целевой файл.
  Источник: src/cse/cstrike/scripts/CseProgression.txt -> runtime/cstrike/scripts/CseProgression.txt
...
$src = Join-Path $Root 'src\cse\cstrike\scripts\CseProgression.txt'
$dst = Join-Path $Root 'runtime\cstrike\scripts\CseProgression.txt'
...
Write-Output 'OK: cstrike/scripts/CseProgression.txt'
