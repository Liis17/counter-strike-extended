...
- Якорь задаёт только точку привязки, но обычно **не** центрирует блок по его ширине: у `top_center`
  с `x = 0` элемент начинается от центра и растёт вправо (для right-aligned элементов — влево).
  Исключения — `TeamBar` и `XPBar`, которые сами трактуют layout point как горизонтальный центр.
  Для остальных элементов, которые нужно визуально отцентрировать, задайте `x = -w/2`; это
  константа, от разрешения она не зависит. То же касается декораций (`Decoration_ResolveTopLeft()`
  не зеркалит по w/h).
