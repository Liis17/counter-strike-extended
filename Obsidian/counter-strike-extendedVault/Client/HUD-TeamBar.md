...
**Статус: план, не реализовано.** Заметка описывает целевой дизайн и порядок работ.
...
3. Клиент раз в N секунд пробует `gRenderAPI.GL_LoadTexture("cache/avatars/<id64>.tga", NULL, 0,
   TF_IMAGE|TF_NOMIPMAP|TF_CLAMP)`, кеширует `id64 -> texnum`, backoff при неудаче,
   `GL_FreeTexture` при переполучении (кеш движка по имени).
