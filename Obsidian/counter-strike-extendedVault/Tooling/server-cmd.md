...
`tools/prepare_server_maps.ps1` перемешивает пул. Поэтому в течение одного запуска карты не
повторяются, пока не закончится полный цикл. Явный `server.cmd de_dust2` меняет только стартовую
карту и также получает случайный порядок следующих карт.

`server.cfg` задаёт `mp_winlimit 6`, `mp_windifference 1`, `mp_maxrounds 0` и `mp_timelimit 0`: карта завершается после
шести побед одной команды, затем ReGameDLL переходит к следующей карте из перемешанного cycle.
...
лимит времени, поэтому переход выполняется штатным `ChangeLevel` ReGameDLL и не сбивает указатель случайного
`mapcycle`. При загрузке следующей карты `cse_map_change.cfg` восстанавливает матч до шести побед.
