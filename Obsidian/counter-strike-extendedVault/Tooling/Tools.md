...
- `tools/install_yapb_map_configs.ps1` — по `runtime/cstrike/maps/*.bsp` создаёт отсутствующие per-map конфиги YaPB в `src/cse/` и копирует их в runtime
...
| `install_yapb_map_configs.ps1 [-DryRun] [-Root <path>]` | Находит `.bsp`-карты, создаёт отсутствующие `yb_difficulty 0` в `src/cse/yapb/conf/maps/` и копирует все `.cfg` в YaPB runtime |
